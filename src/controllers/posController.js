...
const getPos = async (req, res) => {
  try {
    ...
    res.render("pos", { products, customers, role, firstname, avt });
  } catch (err) {
    console.error(err);
...
const getProduct = async (req, res) => {
  try {
    ...
    // Check if the payload starts with a number
    const startsWithNumber = /^[0-9]/.test(payload);

    // If it starts with a number, remove leading zeros
    if (startsWithNumber) {
