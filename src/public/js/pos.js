    ...
    cartData.forEach(function (item, index) {
      ...
      cartTable.append(`
            <tr>
                ...
                <td><input type="number" value="${cartState[item.productName]
...
$(document).on("click", ".btn-add-card", function () {
  ...
  const qtyStock = $(this).closest(".media").find("h5 span").text();
  const isQtyStockValid = parseInt(qtyStock) > 0;
  if (existingItem) {
    const newQuantity =
    ...
    if (isQtyStockValid) {
      $.post("/pos/addToCart", { productName, productPrice, productId }, function (data) {
        displayShoppingCart();
        setupDeleteProductListener();
      });
    } else {
    ...
    fetch("/pos/getProduct", {
      ...
      .then((data) => {
        ...
        payload.forEach((item, index) => {
          console.log(item);
          let retailPriceFormatted = formatCurrency(item.retailPrice);
          console.log(retailPriceFormatted);
                    ...
                    <div class="mb-2 w-50">
                        <div class="media">
                            ...
                            <div class="media-body ml-3">
                                ...
                                <h5 style="font-size: 12px;" id="qtyStock">Hiện còn: <span>${item.quantityInStock}</span> </h5>
                                <h7 hidden class="mb-2">${item._id}</h7>
                                <p class="font-15">${retailPriceFormatted}</p>
  ...
  if (cartData && cartData.length > 0) {
    if (checkFullData() == true) {
      ...
      cartTable.each(function () {
        ...
        const productId = $(this).find("td:nth-child(4)").text();
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          $.ajax({
            ...
            success: function (data) {
              console.log(data);
              $("#pri_invo a").attr("href", `/order/download-invoice/${data._id}`);
              ...
              $("#oldTotalAmountInvoice").text(formatCurrency(data.total_amount + data.total_amount * data.discount/100))
